	...
	try {
		...
		if (state.assetPath.endsWith('mention_default.webp') && !fs.existsSync(assetPath)) {
			...
			if (fs.existsSync(defaultStickerPath)) {
				...
				fs.writeFileSync(assetPath.replace('.webp', '.txt'), 'Figurinha de menção padrão não disponível');
			}
		}
	} catch (e) {
		console.warn('Falha ao garantir figurinha padrão:', e?.message || e);
	}
}
...
async function handleMentionDetection(sock, chatId, message) {
	try {
		...
		if (!state.assetPath) {
			await sock.sendMessage(chatId, { text: 'Oi' }, { quoted: message });
			return;
		}
		const assetPath = path.join(__dirname, '..', state.assetPath);
        ...
        try {
            ...
            else payload.text = 'Oi';
            await sock.sendMessage(chatId, payload, { quoted: message });
        } catch (e) {
            await sock.sendMessage(chatId, { text: 'Oi' }, { quoted: message });
        }
	} catch (err) {
		console.error('Erro em handleMentionDetection:', err);
	}
}

async function mentionToggleCommand(sock, chatId, message, args, isOwner) {
	if (!isOwner) return sock.sendMessage(chatId, { text: 'Apenas o Dono ou Sudo podem usar este comando.' }, { quoted: message });
	const onoff = (args || '').trim().toLowerCase();
	if (!onoff || !['on','off'].includes(onoff)) {
		return sock.sendMessage(chatId, { text: 'Uso: .mention on|off' }, { quoted: message });
	}
	const state = loadState();
	state.enabled = onoff === 'on';
	saveState(state);
	return sock.sendMessage(chatId, { text: `Resposta a menção ${state.enabled ? 'ativada' : 'desativada'}.` }, { quoted: message });
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
	...
	if (!qMsg) return sock.sendMessage(chatId, { text: 'Responda a uma mensagem ou mídia (figurinha/imagem/vídeo/áudio/documento).' }, { quoted: message });
	...
	else return sock.sendMessage(chatId, { text: 'Não suportado. Responda a texto/figurinha/imagem/vídeo/áudio/documento.' }, { quoted: message });
	...
	if (type === 'text') {
		...
		try {
			...
			return sock.sendMessage(chatId, { text: 'Falha ao baixar mídia.' }, { quoted: message });
		}
	}

	// Size limit 1MB
	if (buf.length > 1024 * 1024) {
		return sock.sendMessage(chatId, { text: 'Arquivo muito grande. Máximo 1 MB.' }, { quoted: message });
	}
    ...
    try {
        ...
        console.warn('Falha ao limpar assets anteriores:', e?.message || e);
    }
	...
	try { fs.writeFileSync(outPath, buf); } catch (e) {
		...
		return sock.sendMessage(chatId, { text: 'Falha ao salvar arquivo.' }, { quoted: message });
	}
	...
	saveState(state);
	return sock.sendMessage(chatId, { text: 'Mídia de resposta a menção atualizada.' }, { quoted: message });
}
