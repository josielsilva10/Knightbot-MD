...
const messageTemplates = {
    ...
    success: (text, imageUrl) => ({
        image: { url: imageUrl },
        caption: "GERADO PELO KNIGHT-BOT",
        contextInfo: channelInfo
    })
};
...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        ...
        try {
            ...
            console.error('Erro no gerador de texto:', error);
            await sock.sendMessage(chatId, messageTemplates.error(`Erro: ${error.message}`));
        }
    } catch (error) {
        console.error('Erro no comando textmaker:', error);
        await sock.sendMessage(chatId, messageTemplates.error("Ocorreu um erro. Por favor, tente novamente mais tarde."));
    }
}
