...
function loadAntibadwordConfig(groupId) {
    try {
        ...
        console.error('❌ Erro ao carregar configuração antibadword:', error.message);
        return {};
    }
}
...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*CONFIGURAÇÃO ANTIBADWORD*\n\n*.antibadword on*\nAtiva o antibadword\n\n*.antibadword set <ação>*\nDefine ação: delete/kick/warn\n\n*.antibadword off*\nDesativa o antibadword neste grupo`
        }, { quoted: message });
    }
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword já está ativado para este grupo*' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '*AntiBadword foi ativado. Use .antibadword set <ação> para personalizar a ação*' }, { quoted: message });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword já está desativado para este grupo*' }, { quoted: message } );
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '*AntiBadword foi desativado para este grupo*' }, { quoted: message } );
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '*Ação inválida. Escolha: delete, kick ou warn*' }, { quoted: message } );
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `*Ação do AntiBadword definida para: ${action}*` }, { quoted: message } );
    }

    return sock.sendMessage(chatId, { text: '*Comando inválido. Use .antibadword para ver o uso*' }, { quoted: message } );
}
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (!antiBadwordConfig?.enabled) {
        console.log('Antibadword não está ativado para este grupo');
        return;
    }
   ...
   // console.log('Palavrão detectado em:', userMessage);
    ...
    if (!bot?.admin) {
       // console.log('Bot não é admin, não pode tomar ação');
        return;
    }
    ...
    if (participant?.admin) {
        //console.log('Remetente é admin, pulando ação');
        return;
    }
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `*@${senderId.split('@')[0]} palavrões não são permitidos aqui*`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} foi expulso por usar palavrões*`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `*@${senderId.split('@')[0]} foi expulso após 3 avisos*`,
                        mentions: [senderId]
                    });
                } catch (error) {
                    console.error('Erro ao expulsar usuário após avisos:', error);
                }
            } else {
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} aviso ${warningCount}/3 por uso de palavrões*`,
                    mentions: [senderId]
                });
            }
            break;
