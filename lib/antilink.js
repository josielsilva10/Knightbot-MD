	...
	try {
		...
		switch (action) {
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} foi expulso por enviar links\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} foi expulso após ${WARN_COUNT} avisos\`\`\``,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} aviso ${warningCount}/${WARN_COUNT} por enviar links\`\`\``,
						mentions: [sender]
					});
				}
				break;
